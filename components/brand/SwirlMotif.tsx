...
/**
 * The brand "zen" swirl, blown up as a slowly-rotating abstract motif.
 * Uses the locked logo gradient — purely decorative, aria-hidden.
 */
export function SwirlMotif({ className }: { className?: string }) {
    ...
    <div className={cn("pointer-events-none select-none", className)} aria-hidden>
      ...
      >
        <defs>
          ...
          <filter id="swirl-glow" x="-60%" y="-60%" width="220%" height="220%">
        ...
        >
          <path d={SWIRL_OUTER} fill="url(#swirl-motif-g1)" />
        </motion.g>
        <motion.g
          style={{ transformOrigin: "47px 47px" }}
          animate={reduce ? undefined : { rotate: -360 }}
        ...
        >
          <path d={SWIRL_INNER} fill="url(#swirl-motif-g2)" />
        </motion.g>
      </svg>
    </div>
  );
